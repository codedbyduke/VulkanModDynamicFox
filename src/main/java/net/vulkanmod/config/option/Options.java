...
//                            window.toggleFullScreen();
    ...
    public static OptionBlock[] getFoxOpts() {
    return new OptionBlock[]{
        new OptionBlock("", new Option[]{
            new SwitchOption(
                Component.translatable("vulkanmod.options.dynamicfox.enabled"),
                value -> config.dynamicFoxEnabled = value,
                () -> config.dynamicFoxEnabled
            ).setTooltip(Component.translatable("vulkanmod.options.dynamicfox.enabled.tooltip")),

            new CyclingOption<>(
                Component.translatable("vulkanmod.options.dynamicfox.behavior"),
                Config.FoxBehavior.values(),
                value -> config.dynamicFoxBehavior = value,
                () -> config.dynamicFoxBehavior
            )
            .setTranslator(value -> Component.translatable("vulkanmod.options.dynamicfox.behavior." + value.name().toLowerCase()))
            .setTooltip(Component.translatable("vulkanmod.options.dynamicfox.behavior.tooltip")),

            new CyclingOption<>(
                Component.translatable("vulkanmod.options.dynamicfox.spawnrate"),
                new Integer[]{0, 1, 2, 5, 10, 20, 50, 100},
                value -> config.dynamicFoxSpawnRate = value,
                () -> config.dynamicFoxSpawnRate
            )
            .setTranslator(value -> Component.literal(value.toString()))
            .setTooltip(Component.translatable("vulkanmod.options.dynamicfox.spawnrate.tooltip")),

            new SwitchOption(
                Component.translatable("vulkanmod.options.dynamicfox.ambientnoise"),
                value -> config.dynamicFoxAmbientNoise = value,
                () -> config.dynamicFoxAmbientNoise
            ).setTooltip(Component.translatable("vulkanmod.options.dynamicfox.ambientnoise.tooltip")),
        })
    };
}
